	...
	for {
		...
		if lastIndexedSlot > minimumSlot {
			...
			if lastIndexedSlot == 0 {
				s.logger.Warn("no last indexed slot found, starting from most recent slot (less 100 for safety) and skipping backfill", zap.Uint64("fromSlot", minimumSlot))
...
func (s *SolanaIndexer) handleMessage(ctx context.Context, msg *pb.SubscribeUpdate) {
	...
	if accUpdate != nil {
		...
		if err != nil {
			...
			if insertErr := insertUnprocessedTransaction(ctx, s.pool, txSig.String(), accUpdate.Slot, err.Error()); insertErr != nil {
