...
func TestGetUndisbursedChallenges(t *testing.T) {
	...
	status, body := testGet(t, "/v1/challenges/undisbursed")
	...
	status, body = testGetWithWallet(t,
	...
	status, body = testGet(t, "/v1/challenges/undisbursed?limit=1&offset=1")
	assert.Equal(t, 200, status)
	...
	status, _ = testGet(t, "/v1/challenges/undisbursed?limit=invalid")
	assert.Equal(t, 400, status)

	status, _ = testGet(t, "/v1/challenges/undisbursed?offset=invalid")
	assert.Equal(t, 400, status)

	status, _ = testGet(t, "/v1/challenges/undisbursed?completed_blocknumber=invalid")
	assert.Equal(t, 400, status)
}
