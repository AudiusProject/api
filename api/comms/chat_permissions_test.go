...
func TestChatPermissions(t *testing.T) {
	...
	{
		err := chatSetPermissions(pool, ctx, int32(user1Id), ChatPermissionFollowees, nil, nil, time.Now())
		require.NoError(t, err)
	...
	{
		err := chatSetPermissions(pool, ctx, int32(user1Id), ChatPermissionTippers, nil, nil, time.Now())
		require.NoError(t, err)
	...
	{
		err := chatSetPermissions(pool, ctx, int32(user1Id), ChatPermissionNone, nil, nil, time.Now())
		require.NoError(t, err)
	...
	{
		err := chatSetPermissions(pool, ctx, int32(user1Id), ChatPermissionAll, nil, nil, time.Now())
		require.NoError(t, err)
	}
	...
	{
		err := chatSetPermissions(pool, ctx, int32(user1Id), ChatPermissionNone, nil, nil, time.Now().Add(-time.Hour))
		require.NoError(t, err)
	}
