...
// Helper function to get new blasts, used by the mutation Validator
func GetNewBlasts(pool *dbv1.DBPools, ctx context.Context, arg GetNewBlastsParams) ([]BlastRow, error) {
	...
	rows, err := pool.Query(ctx, findNewBlasts, arg.UserID)
	if err != nil {
		return nil, err
	}
	...
	rows, err = pool.Query(ctx, `select chat_id from chat_member where user_id = $1`, arg.UserID)
	if err != nil {
		return nil, err
	}
