...
func NewApiServer(config config.Config) *ApiServer {
	...
	for _, g := range []fiber.Router{v1, v1Full} {
		...
		g.Get("/users/:userId/recommended_tracks", app.v1UsersRecommendedTracks)
		...
		g.Get("/tracks/recent_premium", app.v1TracksRecentPremium)
		...
		// Explore
		g.Get("/explore/best_selling", app.v1ExploreBestSelling)
