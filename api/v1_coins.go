...
=======
		SELECT
			...
			JSON_BUILD_OBJECT(
				...
				'totalTradingQuoteFee', COALESCE(artist_coin_pools.total_trading_quote_fee, 0),
				'creatorWalletAddress', COALESCE(artist_coin_pools.creator_wallet_address, ''),
				'reservedAmount', COALESCE(artist_coin_pools.reserved_amount, 0),
				'claimedVestedAmount', COALESCE(artist_coin_pools.claimed_vested_amount, 0),
				'totalUnlockedAmount', COALESCE(artist_coin_pools.total_unlocked_amount, 0),
				'graduationDate', artist_coin_pools.graduation_date
			) AS dynamic_bonding_curve,
			COALESCE(artist_coin_stats.updated_at, artist_coins.created_at) as updated_at
		FROM artist_coins
		LEFT JOIN artist_coin_stats
