...
ORDER BY
    CASE WHEN $2::text = 'date' AND $3::text = 'asc' THEN uth.created_at END ASC,
    CASE WHEN $2::text = 'date' AND $3::text = 'desc' THEN uth.created_at END DESC,
...
func (q *Queries) GetUserUsdcTransactionsCount(ctx context.Context, userID int32) (int64, error) {
	row := q.db.QueryRow(ctx, getUserUsdcTransactionsCount, userID)
	var count int64
	err := row.Scan(&count)
	return count, err
}
