...
func testAppWithFixtures(t *testing.T) *ApiServer {
	...
	database.SeedTable(app.pool.Replicas[0], "aggregate_plays", testdata.AggregatePlays)
	...
	database.SeedTable(app.pool.Replicas[0], "comment_threads", testdata.CommentThread)
	...
	database.SeedTable(app.pool.Replicas[0], "playlist_routes", testdata.PlaylistRoutesFixtures)
	database.SeedTable(app.pool.Replicas[0], "playlist_trending_scores", testdata.PlaylistTrendingScores)
