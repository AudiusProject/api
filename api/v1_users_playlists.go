...
type GetUsersPlaylistsParams struct {
	...
	FilterPlaylists string `query:"filter_playlists" default:"all" validate:"oneof=all public private"`
	SortDirection   string `query:"sort_direction" default:"desc" validate:"oneof=asc desc"`
}

func (app *ApiServer) v1UserPlaylists(c *fiber.Ctx) error {
	params := GetUsersPlaylistsParams{}
	...
	playlistFilter := "(p.is_private = false OR p.playlist_owner_id = @my_id)"
	switch params.FilterPlaylists {
	case "public":
		playlistFilter = "p.is_private = false"
	case "private":
		playlistFilter = "p.is_private = true"
	}
